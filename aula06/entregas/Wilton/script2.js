...
function formatarTempo(milissegundos_para_fim_da_tarefa) 
{
    if(milissegundos_para_fim_da_tarefa <= 0) 
    {
        const segundos = Math.floor(milissegundos_para_fim_da_tarefa*-1 / 1000);
        const minutos = Math.floor(segundos / 60);
        const segundosRestantes = Math.floor (segundos % 60);

        return 'Tempo Esgotado! ' + `(${minutos.toString().padStart(2, '0')}:${segundosRestantes.toString().padStart(2, '0')})`;
    
    }
    
    const segundos = Math.floor(milissegundos_para_fim_da_tarefa / 1000);
    const minutos = Math.floor(segundos / 60);
    const segundosRestantes = Math.floor (segundos % 60);
